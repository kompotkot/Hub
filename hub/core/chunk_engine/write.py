...
def write_array(
    ...
    Args:
        array (np.ndarray): Array to be chunked/written. Batch axis (`array.shape[0]`) is optional, if `array` does have
            a batch dimension, you should pass the argument `batched=True`.
        ...
        to_bytes (Callable): Callable that flattens an array into bytes. Must accept an `np.ndarray` as it's argument 
            and return `bytes`.
